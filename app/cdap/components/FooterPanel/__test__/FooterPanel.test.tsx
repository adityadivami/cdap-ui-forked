...
describe("Test Footer Panel Component", () => {
  ...
  it("Should render the component with correct props", () => {
    const outlinedLabelElement = screen.getByTestId(
      /footerpanel-outlined-label/i
    );

    // Label inside OutlinedLabelElement to be 42
    expect(outlinedLabelElement).toHaveTextContent("42");

    const simpleLabelElement = screen.getAllByTestId(
      /footerpanel-simple-label/i
    );

    // Check if the row count and column count is same as provided props 66 and 6
    expect(simpleLabelElement[0]).toHaveTextContent(
      `${T.translate(`${PREFIX}.currentData`)} - 66 ${T.translate(
        `${PREFIX}.rows`
      )} ${T.translate(`features.WranglerNewUI.common.and`)} 6 ${T.translate(
        `${PREFIX}.columns`
      )}`
    );
  });
  ...
  it("Should render all the child elements correctly in the parent component", () => {
    ...
    const viewPanelTabParent = screen.getByTestId(
      /footer-panel-column-view-panel-tab-wrapper/i
    );
    ...
    const directivesElement = screen.getByTestId(
      /footer-panel-directives-tab/i
    );
    ...
    const recipeStepsTabElement = screen.getByTestId(
      /footer-panel-recipe-steps-tab/i
    );
