import {
  ...
  const SelectColumnsList = (props) => {
    ...
    const onSelect = (event, column) => {
      if(event.target.checked){
        ...
        if(indexOfUnchecked > -1){
          ...
          let remaining_col = clone_columns.splice(indexOfUnchecked, 1)
    ...
    const handleDisableCheckbox = (column) => {
     const multiSelect =  multipleColumnSelected.filter(el=>el.value == functionName && el.isMoreThanTwo)
                  ...
                  if (directiveFunctionSupportedDataType.includes('all')) {
                      return (
                        <TableRow className={classes.recipeStepsTableBodyRowStyles} key={index}>
                          ...
                          >
                            <FormControlLabel disabled={selectedColumns.filter(el=>el.label === eachColumn.label).length || !handleDisableCheckbox(eachColumn) ? false : true} control={<Checkbox color="primary" checked={selectedColumns.filter(el=>el.label === eachColumn.label).length ? true : false} onChange={(e)=>onSelect(e, eachColumn)}/>} label=""/>
                          </TableCell>
                          <TableCell
