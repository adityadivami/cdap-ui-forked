...
export default function ConnectionList() {
  ...
  // console.log(snackbarStatus, 'snackbarStatus');

  DataPrepStore.subscribe(() => {
    console.log(DataPrepStore.getState().dataprep.snackbarStatus, ' ---- store');
  });
  ...
  const fetchEntities = async (connectionName: string, url = pathFromUrl) => {
  ...
  const selectedTabValueHandler = (entity: IRecords, index: number) => {
    ...
    setDataForTabs((currentData) => {
      ...
      if (index === 0) {
        ...
        fetchEntities(entity.name as string).then((res) => {
  ...
  return (
    <Box data-testid="data-sets-parent" className={classes.connectionsListContainer}>
      ...
      {snackbarStatus.open && (
        <Snackbar description={snackbarStatus.message} isSuccess={snackbarStatus.isSuccess} />
      )}
    </Box>
  );
}
